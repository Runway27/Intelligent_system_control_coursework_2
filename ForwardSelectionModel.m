...
%% 2017-2018 = test
%%
%% You may choose different splits of 2014-2016 data when forming the traniing and validation datasets if you wish.
...
for k=1:1:Nvar
    ...
    SelVar=[SelVar AvailVar(minIdx)];  %add the variable with the lowest RMSE to the selected variables.
...
for k=1:1:Kopt
    fprintf('%2d: %-30s    RMSE: %2.2f \n',k, string(vnames(SelVar(k))),RMSEBest_Val(k));
end
